    ...
    stages {
        ...
        stage('Docker build') {
            steps {
                ...
                script {
                    app = docker.build("thomasilliet/pyenv-tox:${env.BUILD_ID}")
                }
            }
        }
        ...
        stage('Docker push Weekly') {
            ...
            steps {
                script {
                    commitId = sh(returnStdout: true, script: 'cd pyenv ; git rev-parse HEAD')
                    docker.withRegistry('https://registry.hub.docker.com', 'ca19e01b-db1a-43a3-adc4-46dafe13fea2') {
                        app.push("weekly")
                    }
        ...
        stage('Cleanup') {
            steps {
                script {
                    ...
                    sh("docker rmi -f debian:stable")
                    sh("docker rmi -f registry.hub.docker.com/thomasilliet/pyenv-tox:latest")
                    sh("docker rmi -f registry.hub.docker.com/thomasilliet/pyenv-tox:monthly")
                    sh("docker rmi -f registry.hub.docker.com/thomasilliet/pyenv-tox:weekly")
                    sh("docker rmi -f registry.hub.docker.com/thomasilliet/pyenv-tox:$currentCommitID")
                    sh("docker rmi -f thomasilliet/pyenv-tox:${env.BUILD_ID}")
                    deleteDir()
                    cleanWs()
                }
            }
...
def getRemoteCommitID() {
    return sh(returnStdout: true, script: 'cd pyenv ; git rev-parse HEAD')
}
